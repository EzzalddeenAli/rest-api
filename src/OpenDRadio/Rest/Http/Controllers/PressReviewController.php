<?php
namespace OpenDRadio\Rest\Http\Controllers;

use Cache;
use Carbon;
use Input;
use Response;
use OpenDRadio\Radio\Models\PressReviewModel;
...
class PressReviewController extends BaseController
{
    ...
    public function latest()
    {
        ...
        if (null !== $stationName = Input::get('station')) {
            ...
            $models = Cache::remember($cacheKey, $this->cacheDuration, $fromDate, $toDate, $sort, function() use ($station, $fromDate, $sort) {
                return PressReviewModel::where('enabled', true)->where('station_id', $station->getId())
            ...
            $models = Cache::remember($cacheKey, $this->cacheDuration, function() use ($fromDate, $sort) {
                return PressReviewModel::where('enabled', true)->where('publicated_at', '>=', $fromDate)
    ...
    public function get($id)
    {
        ...
        $model = Cache::remember($cacheKey, $this->cacheDuration, function() use ($id) {
            return PressReviewModel::where('enabled', true)->where('_id', $id)
                        ->get()
                        ->first();
        });
